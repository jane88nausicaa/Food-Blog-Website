...
function users_online(){
    if(isset($_GET['onlineusers'])) {
        global $connection;
        // when get to the users_online function, db not include
        // global $connection is not available
        if(!$connection) {
            session_start();
            include("../includes/db.php");
            
            // if open Chrome, one session id
            // if open Firefox, another session id
            $session = session_id();
            $time = time();
...
function checkStatus($table, $col, $status) {
    global $connection;
...
function login_user($username, $password) {
    ...
    while($row = mysqli_fetch_array($select_user_query)) { 
        ...
        if(password_verify($password, $db_user_password)) {
            ...
            $_SESSION['lastname'] = $db_user_astname;
            $_SESSION['user_role'] = $db_user_role;

            redirect("/sweetTaste/admin");
        } else {
            return false;
        }
    }
